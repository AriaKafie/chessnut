...
namespace Chess {
	...
	void WhiteCaptureGenerator::generate() {
		...
		for (uint64_t from_map = Board::bitboards[WHITE_BISHOP] & pin_D; from_map; from_map = blsr(from_map)) {
			int from = tzcnt(from_map);
		...
		for (uint64_t from_map = Board::bitboards[WHITE_ROOK] & pin_O; from_map; from_map = blsr(from_map)) {
			int from = tzcnt(from_map);
	...
	uint64_t WhiteCaptureGenerator::squares_seen_by_black() {
