...
int search(int alpha, int beta, int depth, int ply_from_root) {
  ...
  for (int i = 0; i < moves.size(); i++)
  {
    ...
    int eval = -search<!SideToMove>(-beta, -alpha, depth - 1 - depth_reduction[i] + extension, ply_from_root + 1);
    if (eval > alpha && depth_reduction[i] && (depth - 1 + extension > 0))
      eval = -search<!SideToMove>(-beta, -alpha, depth - 1 + extension, ply_from_root + 1);
...
Move best_move(uint64_t thinktime) {
  ...
  for (int depth = 1; depth < MAX_PLY && !search_cancelled; depth++)
  {
    ...
    {
      do_move<SideToMove>(moves[i]);
      eval = -search<!SideToMove>(-INFINITE, -alpha, depth - 1 - reduction[i], 0);
      if (eval > alpha && reduction[i])
        eval = -search<!SideToMove>(-INFINITE, -alpha, depth - 1, 0);
      undo_move<SideToMove>(moves[i]);
    ...
    for (int d = 1; d <= depth; d++) {
      if (Position::white_to_move()) {
        ...
        for (int i = 0; i < moves.size(); i++) {
          ...
          int eval = -search<BLACK>(-INFINITE, -alpha, d - 1 - reduction[i], 0);
          if (eval > alpha && reduction[i])
            eval = -search<BLACK>(-INFINITE, -alpha, d - 1, 0);
          if (eval > alpha) {
            alpha = eval;
            best_move = moves[i];
          }
        ...
        for (int i = 0; i < moves.size(); i++) {
          ...
          int eval = -search<WHITE>(-INFINITE, -alpha, d - 1 - reduction[i], 0);
          if (eval > alpha && reduction[i])
            eval = -search<WHITE>(-INFINITE, -alpha, d - 1, 0);
          if (eval > alpha) {
            alpha = eval;
            best_move = moves[i];
          }
